...
context("The Choice Page", ()=>{
    beforeEach(()=> {
      ...
      it('contains some welcome text', () => {
        cy.contains('CHOOSE ONE FOR DETAILS');
      });
      
      it('displays only loved cards clickable id\'s images', () => { 
        cy.visit('http://localhost:3000/beender/gallery');
       let love = [1,2,3,4,5,6,7];
       love.map(e => cy.get('#loveButton').click());
           cy.get('#gallery').contains('GO TO SELECTION').click();
           cy.url().should('include', '/choice');  
      });

      it('contains buttons to come back to gallery', () => {
        cy.get('#backToGalleryButton').contains('or back to gallery');
      }); 
});
